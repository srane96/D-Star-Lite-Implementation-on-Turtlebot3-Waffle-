...
class Graph:
    ...
    def d_star_lite_algo(self, rob_x, rob_y, goal_x, goal_y,bg):
        ...
        self.open_length += 1
        curr_node = goal_node
        print("Open before:",self.open_list)
        while not curr_node == start_node and not len(self.open_list) == 0:
            ...
            #print("Next",curr_node)
            #print("Parent",self.nodes[curr_node].parent)
        print("Done",curr_node)
        current_path = []
        ...
        print("Open Length: ",self.open_length)
    ...
    def get_new_key(self, node, start):
        key = min(self.nodes[node].g_cost, self.nodes[node].rhs) + 5 * self.h(node, start)
        return key
    ...
    def replan(self,rob_x, rob_y, goal_x, goal_y,current_path,bg):
        ...
        new_path = []
        print("curr_node",curr_node,start_node)
        print(self.nodes[curr_node].parent)
        count = 0
        while not self.nodes[curr_node].parent == None:
...
def initiate():
    ...
    #cv2.waitKey(0)
    #cv2.destroyAllWindows()
    current_path = graph.d_star_lite_algo(-300,0,-200,0,bg)
